 ...
 */
const groupIntoLines = (str, maxWidth, indents, { breakLongWords, textDirection, newlineBeforeLongWords, useVisualWidth, whitespaceNormalize, whitespaceChar, padToMaxWidth, whitespaceMaintainWideSpace }) => {
  ...
  for (let n = 0; n < split.length; ++n) {
    ...
    while (true) {
      ...
      // Checks whether the item is entirely whitespace.
      const isWhitespace = item.match(/\s/) != null
      // Checks whether the item is a wide space character.
      const isWideSpace = item.match(/\u3000/) != null
      // Whether to normalize the string to a single whitespace character.
      const useWhitespaceCharacter = isWhitespace && whitespaceNormalize && (!isWideSpace || !whitespaceMaintainWideSpace)
 ...
 */
const getMaxWidth = ({ maxWidth, indentAmount, throwOnError }, indents) => {
  const [indentValues] = indents
