...
/**
 * Splits a string on whitespace (keeping the whitespace).
 * 
 * If whitespace is being normalized, each instance of consecutive whitespace characters
 * is returned as a single string. Otherwise, each character is a separate string.
 */
const splitOnWords = (str, whitespaceNormalize = true) => {
  if (whitespaceNormalize) {
    return str.split(/([^\S\u00A0]+)/)
  }
  else {
    return str.split(/([^\S\u00A0])/)
 ...
 */
const splitOnWordsOrCJK = (str, whitespaceNormalize = true) => {
  const segments = []
  const split = splitOnWords(str, whitespaceNormalize)
